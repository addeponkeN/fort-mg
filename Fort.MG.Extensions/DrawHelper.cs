...
public static class DrawHelper
{
    ...
    {
        DrawLine(start, end, color, size, 0f);
    }

    /// <summary>
    /// Draws a straight line between two points.
    /// </summary>
    public static void DrawLine(Vector2 start, Vector2 end, Color color, float size, float layer)
    {
        Vector2 edge = end - start;
        float angle = (float)Math.Atan2(edge.Y, edge.X);
        float length = edge.Length();
        Vector2 origin = new Vector2(0f, 0.5f);
        Sb.Draw(Pixel, start, PixelSource, color, angle, origin, new Vector2(length, size), SpriteEffects.None, layer);
    }
